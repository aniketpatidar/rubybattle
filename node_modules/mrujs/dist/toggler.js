...
export class Toggler {
    constructor() {
        this.boundEnableElement = this.enableElement.bind(this);
        this.boundDisableElement = this.disableElement.bind(this);
    }
    get enableElementConditions() {
        return [
   ...
   */
    enableFormElements(form) {
        const elements = findFormElements(form, window.mrujs.querySelectors.formEnableSelector.selector);
        elements.forEach((el) => {
...
function findFormElements(form, selector) {
    if (match(form, { selector: 'form' })) {
        return Array.from(form.elements).filter((el) => match(el, { selector }));
    }
